/*
 *
 * ©K. D. Hedger. Mon 13 Jan 16:32:46 GMT 2025 keithdhedger@gmail.com

 * This file (image.c) is part of LFSTray.
...
int image_tint(XImage *image,XColor *color,CARD8 alpha)
{
	switch (image->bits_per_pixel)
		...
		default:
			{
				fprintf(stderr,"image_tint() called with unsupported depth %d\n",image->bits_per_pixel);
				exit(-1);
			}
...
int image_compose(XImage *image,XImage *bg,CARD8 *mask)
{
	switch (image->bits_per_pixel)
		...
		default:
			{
				fprintf(stderr,"image_compose() called with unsupported depth %d\n",image->bits_per_pixel);
				exit(-1);
			}
			return FAILURE;
		}
